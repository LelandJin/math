...
class CarbonBuildMacOS(buildName: String, configType: String, preset: String) : BuildType({
    ...
    vcs {
        root(AbsoluteId("Carbon_Math_2_MathFeatureKotlin"),"+:. => %github_checkout_folder%")
    ...
    triggers {
        vcs {
            triggerRules = "+:root=${AbsoluteId("Carbon_Math_2_MathFeatureKotlin").id}:."
    ...
    features {
        pullRequests {
            vcsRootExtId = "${AbsoluteId("Carbon_Math_2_MathFeatureKotlin")}"
            provider = github {
                authType = token {
